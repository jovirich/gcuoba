...
import { DuesSchemeSchema } from '../modules/finance/schemas/dues-scheme.schema';
import { DuesInvoiceSchema } from '../modules/finance/schemas/dues-invoice.schema';
import { WelfareCaseSchema } from '../modules/welfare/schemas/welfare-case.schema';
import { WelfareContributionSchema } from '../modules/welfare/schemas/welfare-contribution.schema';
import { WelfarePayoutSchema } from '../modules/welfare/schemas/welfare-payout.schema';
import { PaymentSchema } from '../modules/finance/schemas/payment.schema';
import { PaymentReceiptSchema } from '../modules/finance/schemas/payment-receipt.schema';
import { UserSchema } from '../modules/users/schemas/user.schema';
import { BranchSchema } from '../modules/branches/schemas/branch.schema';
import { ClassSetSchema } from '../modules/classes/schemas/class-set.schema';
import { BranchMembershipSchema } from '../modules/memberships/schemas/branch-membership.schema';
import { ClassMembershipSchema } from '../modules/memberships/schemas/class-membership.schema';
import { RoleSchema } from '../modules/roles/schemas/role.schema';
import { RoleAssignmentSchema } from '../modules/role-assignments/schemas/role-assignment.schema';
import { AnnouncementSchema } from '../modules/dashboard/schemas/announcement.schema';
import { EventSchema } from '../modules/dashboard/schemas/event.schema';
import { HouseSchema } from '../modules/houses/house.schema';
...
async function run() {
    ...
    const schemeModel = connection.model('DuesScheme', DuesSchemeSchema);
    const invoiceModel = connection.model('DuesInvoice', DuesInvoiceSchema);
    const welfareCaseModel = connection.model('WelfareCase', WelfareCaseSchema);
    const welfareContributionModel = connection.model(
        'WelfareContribution',
        WelfareContributionSchema,
    );
    const welfarePayoutModel = connection.model(
        'WelfarePayout',
        WelfarePayoutSchema,
    );
    const userModel = connection.model('User', UserSchema);
    const branchModel = connection.model('Branch', BranchSchema);
    const classModel = connection.model('ClassSet', ClassSetSchema);
    const branchMembershipModel = connection.model(
        'BranchMembership',
        BranchMembershipSchema,
    );
    const classMembershipModel = connection.model(
        'ClassMembership',
        ClassMembershipSchema,
    );
    const paymentModel = connection.model('Payment', PaymentSchema);
    const receiptModel = connection.model(
        'PaymentReceipt',
        PaymentReceiptSchema,
    );
    const roleModel = connection.model('Role', RoleSchema);
    const roleAssignmentModel = connection.model(
        'RoleAssignment',
    ...
    const eventModel = connection.model('DashboardEvent', EventSchema);
    const houseModel = connection.model('House', HouseSchema);
    ...
    const globalAdminPasswordHash = await bcrypt.hash('Gcuoba2026', 10);
    ...
    const invoice = await invoiceModel.create({
        schemeId: new Types.ObjectId(scheme._id),
        userId: memberUser._id.toString(),
        amount: 50000,
        currency: 'NGN',
        status: 'unpaid',
    ...
    await invoiceModel.updateOne(
        { _id: invoice._id },
        { $set: { paidAmount: 20000, status: 'part_paid' } },
    );
    ...
    await welfareCaseModel.updateOne(
        { _id: welfareCase._id },
        { $set: { totalRaised: 75000, totalDisbursed: 30000 } },
    );
    ...
    console.log(
        'Seeded demo users, branches, roles, memberships, and finance data.',
    );
    console.log(
        'Login with exec@example.com / password for branch executive access.',
    );
    console.log(
        'Login with member@example.com / password for a pending member view.',
    );
    console.log(
        'Login with ejovi.ekakitie@hotmail.com / Gcuoba2026 for global admin access.',
    );
}
