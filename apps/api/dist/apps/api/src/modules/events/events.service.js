...
const event_schema_1 = require("../dashboard/schemas/event.schema");
...
let EventsService = class EventsService {
    ...
    auditLogsService;
    constructor(eventModel, roleAssignmentsService, membershipsService, usersService, notificationsService, auditLogsService) {
    ...
    async update(actorId, id, dto) {
        ...
        await this.ensureWritableScope(actorId, event.scopeType, event.scopeId ?? null);
        ...
        if (event.status === 'published') {
            await this.notifyEventAudience(event, previousStatus === 'published' ? 'updated' : 'published');
        }
    ...
    async notifyEventAudience(event, action) {
        const audience = await this.resolveAudienceUserIds(event.scopeType, event.scopeId ?? null);
...
exports.EventsService = EventsService = __decorate([
    ...
    __param(0, (0, mongoose_1.InjectModel)(event_schema_1.DashboardEvent.name)),
