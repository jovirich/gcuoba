...
let ProfilesController = class ProfilesController {
    ...
    upsert(userId, payload, user) {
        this.ensureSelf(user, userId);
        return this.profilesService.upsert(userId, payload);
    }
    ensureSelf(user, userId) {
        if (!user || user.id !== userId) {
            throw new common_1.ForbiddenException('Cannot access another profile');
        }
    }
};
...
__decorate([
    ...
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, upsert_profile_dto_1.UpsertProfileDto, Object]),
    __metadata("design:returntype", Promise)
